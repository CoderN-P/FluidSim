...
namespace GridDemo1
{
    ...
    public class FluidGrid
    {
        ...
        [Header("Grid Data")] [SerializeField] public Vector2[] velocity; // Velocity field (staggered)
        ...
        public void InitializeVelocities()
        {
            for (int i = 0; i <= width + 1; i++)
            {
                for (int j = 0; j <= height + 1; j++)
                {
                    ...
                    velocity[Idx(i, j)] =
                        new Vector2(Random.Range(-5f, 5f), Random.Range(-5f, 5f)); // Random initial velocity
                }
            }
        }
